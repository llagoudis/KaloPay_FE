...
const IconChart = ({ className, active, color }: { className?: string; active?: boolean; color?: string }) => (
  <svg
    className={cn("h-5 w-5 flex-shrink-0", !color && (active ? "text-blue-600" : "text-gray-500"), className)}
    style={color ? { color } : undefined}
    fill="none"
...
const IconHome = ({ className, active, color }: { className?: string; active?: boolean; color?: string }) => (
  <svg className={cn("h-5 w-5 flex-shrink-0", !color && (active ? "text-blue-600" : "text-gray-500"), className)}
    style={color ? { color } : undefined} fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const adminNavSections = [
  {
    title: "Home",
    SectionIcon: IconHome,
    iconColor: "#0F50DB",
    items: [
      { label: "Dashboard", href: ROUTES.admin.dashboard, Icon: IconChart },
      { label: "Bills", href: ROUTES.admin.bills, Icon: IconDocument },
    ],
  },
  {
    title: "Management",
    ...
    items: [
      ...
      { label: "Top-ups", href: ROUTES.admin.topups, Icon: IconTransaction },
    ],
  },
  {
    title: "Reports",
    SectionIcon: IconChart,
    iconColor: "#0F50DB",
    items: [
      { label: "Payroll", href: ROUTES.admin.reports.payroll, Icon: IconDocument },
      { label: "Monthly", href: ROUTES.admin.reports.monthly, Icon: IconDocument },
      { label: "Period", href: ROUTES.admin.reports.period, Icon: IconDocument },
      { label: "Annual", href: ROUTES.admin.reports.annual, Icon: IconDocument },
    ],
  },
  {
...
export default function Sidebar({ role }: SidebarProps) {
  ...
  const sectionOpen = { Home: homeOpen, Management: managementOpen, Reports: reportsOpen, Administration: administrationOpen };
  const setSectionOpen = (title: string, open: boolean) => {
    if (title === "Home") setHomeOpen(open);
    else if (title === "Management") setManagementOpen(open);
    else if (title === "Reports") setReportsOpen(open);
