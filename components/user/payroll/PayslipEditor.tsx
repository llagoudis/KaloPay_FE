...
import { getPayslip, putPayslip } from "@/lib/api/employer/appFeatures";
...
function seedData(e: PayslipEmployee): PayslipData {
  // Seed the Basic Salary earning from GROSS (not e.salary, which is the NET paid amount for
  // batch rows). Deductions are gross-based, so earnings(gross) − deductions === e.net — keeping
  // the editor's Net Pay equal to BatchDetail's Net. (For gross-basis rows salary === gross.)
  const gross = e.gross;
  const rate = gross / 22 / 8;
  return {
    earnVals: {
      salaryRate: rate.toFixed(2), salary: gross.toFixed(2), cola: "0.00",
...
export default function PayslipEditor({
  ...
  const payslipKey = `${batchRef || "x"}-${e.id}`;
  ...
  // Reconcile with the server after mount (localStorage seeded the initial paint).
  useEffect(() => {
    if (!token) return;
    let alive = true;
    getPayslip(token, payslipKey)
      .then((res) => {
        const d = res?.data as PayslipData | null;
        if (alive && d && typeof d === "object" && Object.keys(d).length) setData(d);
      })
      .catch(() => {});
    return () => { alive = false; };
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      setSaveState("saved");
      notify("Changes autosaved");
      if (token) putPayslip(token, payslipKey, data).catch(() => {});
    }, 600);
    return () => { cancelAnimationFrame(raf); clearTimeout(t); };
  }, [data, storeKey, payslipKey, token]);
 ...
 * `basis` = "gross" (default) treats `salary` as the monthly gross.
 * `basis` = "net" treats `salary` as the real disbursed amount and grosses it up so
 * that gross − deductions === salary exactly (used for batch payments, where the
 * backend only gives the paid amount — Net then reflects the real money, not a guess).
 */
export function derivePayslipEmployee(
  ...
  const totalRate = DERIVE_RATES.incomeTax + DERIVE_RATES.social + DERIVE_RATES.health;
  const gross = basis === "net" ? src.salary / (1 - totalRate) : src.salary;
