...
import { generateMonthlyReport } from "@/lib/api/admin/reports";
...
export default function MonthlyReportsPage() {
  ...
  const mut = useMutation({
    mutationFn: () => generateMonthlyReport(token!, { company, period }),
    onSuccess: () => {
      setSuccess(true);
  ...
  return (
    <div className="w-full space-y-6">
      <div className="rounded-[10px] bg-white p-6">
        <h1 className="text-2xl font-semibold text-[#0E1620]">Monthly Reports</h1>
        <p className="mt-1 text-sm text-gray-500">
          Review monthly payroll and headcount summaries per company.
        </p>
      </div>
